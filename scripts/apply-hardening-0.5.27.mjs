...
async function patchFile(path, transform) {
  ...
  if (updated === source) throw new Error(`${path} was not changed`)
...
function mergeStateJob(baseJob, intendedJob, currentJob) {
  ...
  for (const key of keys) {
    ...
    // may still apply changes to unrelated fields, which prevents pause/resume,
    // counters, verification, and scheduler bookkeeping from erasing each other.
...
function mergeStateJobs(baseJobs, intendedJobs, currentJobs) {
  ...
  for (const currentJob of currentJobs || []) {
    ...
    if (!intendedJob) {
      // Do not let a stale deletion erase a job that changed after the snapshot.
...
await patchFile("scripts/comprehensive-test.mjs", (input) => {
  const test = `async function testStateRebasePreservesConcurrentPause() {
...
`
  let source = replaceOnce(input, "await testParserAndPresets()\n", `${test}await testParserAndPresets()\n`, "comprehensive test call marker")
  source = replaceOnce(source, "await testStateReadRetriesTransientPartialJson()\n", "await testStateReadRetriesTransientPartialJson()\nawait testStateRebasePreservesConcurrentPause()\n", "comprehensive final test marker")
  return source
})
...
await patchFile("scripts/loopd.mjs", (input) => {
  ...
  try {
    ...
    child.kill("SIGTERM")
    const force = setTimeout(() => { try { if (child.exitCode === null) child.kill("SIGKILL") } catch {} }, 1_000)
    force.unref?.()
  } catch {}
}
...
`,
  ...
  while (true) {
    ...
    else runArgs.push("--continue", "--title", sessionTitle)
    ...
    if (!sessionID && code === 0) {
      ...
      if (sessionID) console.log(`[opencode-loopd] pinned session ${sessionID}`)
      else console.warn("[opencode-loopd] could not resolve the created session; the next run will use --continue again")
    }
    ...
    if (code !== 0 && delay === 0 && !(maxRuns > 0 && count >= maxRuns)) {
...
`,
  ...
  assert.deepEqual(runCalls[0].args, [
    "run", "--continue", "--title", runCalls[0].args[3],
    "--model", "opencode/nemotron-3-ultra-free",
    "--agent", "build",
    inlinePrompt,
  ], "loopd must preserve model, agent, quotes, and shell metacharacters as literal arguments")
  assert.match(runCalls[0].args[3], /^OpenCode Loop daemon /)
`,
  ...
  const insertionMarker = '  const failureLog = path.join(temporaryRoot, "failure.jsonl")\n'
  ...
  assert.equal(result.code, 124, `timeout run should exit 124, got ${"${result.code}"}\\n${"${result.stdout}"}\\n${"${result.stderr}"}`)
  assert.match(result.stdout, /timed out after 50ms/)

`
  source = replaceOnce(source, insertionMarker, `${added}${insertionMarker}`, "daemon regression insertion")
...
`,
    ...
    assert.deepEqual(taskRunCalls[0].args, [
      "run", "--continue", "--title", taskRunCalls[0].args[3],
      "--model", "opencode/nemotron-3-ultra-free",
      "--agent", "build",
      "scheduled task prompt",
    ])
...
await patchFile("README.md", (input) => {
  ...
  source = replaceOnce(
    ...
    "The normal `/loop` plugin is session-bound. If OpenCode closes, that TUI/session loop cannot keep running in the background. `opencode-loopd` pins the OpenCode session it selects on startup so later iterations do not drift to a newer unrelated session. Each daemon run is also bounded by `--timeout` (30 minutes by default; use `--timeout 0s` to disable).\n",
...
- Rebased stale Loop state snapshots against the latest persisted job state before writing, preventing concurrent scheduler bookkeeping from erasing unrelated user pause/resume or other state changes.
- Added a deterministic concurrency regression that forces overlapping state reads/writes and proves a concurrent user pause survives a stale scheduler update.
- Hardened \`opencode-loopd\` by pinning the selected OpenCode session across iterations instead of resolving \`--continue\` on every run.
- Added per-run daemon timeouts (30 minutes by default, configurable with \`--timeout\`; \`0s\` disables) and timeout exit code 124.
- Added daemon regressions for session pinning, timeout termination, and scheduled-task timeout propagation.
- Narrowed the OpenCode plugin peer range to \`>=1.4.0 <2\` so a future breaking 2.x release is not treated as automatically compatible.
